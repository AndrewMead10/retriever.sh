...
- Remote embedding server cutover (May 22, 2026): local Sentence Transformers/DenseOn loading was removed. Text embeddings now call the OpenAI-compatible service at `RAG_EMBEDDING_BASE_URL` (default `https://embedding-server.amqm.dev`) with `RAG_EMBEDDING_API_KEY`, model `jinaai/jina-embeddings-v5-text-small-retrieval-mlx`, `task_type` `retrieval.passage` for ingest and `retrieval.query` for search, and 512-d float outputs. Existing Vespa text indexes should be rebuilt/reingested after deployment because the embedding model/dimension changed.
