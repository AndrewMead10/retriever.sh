...
- Polar upgrade flow fix (Mar 18, 2026): existing subscribers no longer start a brand-new Polar checkout when switching plans, because Polar rejects that with "you already have a subscription." Customer-facing upgrade buttons now send existing subscribers into the Polar billing portal instead of silently mutating the subscription server-side, and webhook subscription sync can resolve users by `polar_subscription_id`/`polar_customer_id` when metadata is absent.
...
- Docs download refresh (Feb 7, 2026): legacy `/docs` downloadable `retriever-claude-skill.md` was aligned with project-scoped API routes at the time; this was later superseded by the standalone skills.sh-compatible skill repo.
...
- Env example alignment (Feb 21, 2026): `.env.example` now also defaults `RAG_IMAGE_MODEL_ID` to `google/siglip2-base-patch16-naflex` so new deployments match the SigLIP2-only cutover.
...
- Remote embedding server cutover (May 22, 2026): local Sentence Transformers/DenseOn loading was removed. Embeddings now call the OpenAI-compatible service at `RAG_EMBEDDING_BASE_URL` (default `https://embedding-server.amqm.dev`) with `RAG_EMBEDDING_API_KEY`, model `jinaai/jina-embeddings-v5-text-small-retrieval-mlx`, `task_type` `retrieval.passage` for ingest and `retrieval.query` for search, and 512-d float outputs.
- Multimodal RAG API cutover (May 30, 2026): public RAG ingest/delete routes now use `/api/rag/projects/{project_id}/items` and `/items/{item_id}` instead of `/documents` and `/vectors/{document_id}`. Ingest and query payloads use typed `content`/`input` blocks (`text`, `image_url`, `image_base64`, `audio_url`, `audio_base64`, `video_url`, `video_base64`, `file_url`, `file_base64`); the embedding adapter sends direct strings/URLs/data URIs to the remote embedding server and averages multi-block vectors into one item/query vector. There is intentionally no separate file-upload endpoint. Vespa still uses the `rag_document` schema but now stores `content_blocks` and `primary_modality` summaries alongside the plain text projection used for lexical ranking.
- Agent auth self-serve cutover (May 30, 2026): project RAG routes now require `Authorization: Bearer <project_api_key>` instead of `X-Project-Key`, with project keys stored in `project_api_keys` and shown once on create/rotation. The `/api-keys` UI creates/revokes account-level management keys (`retr_mgmt_...`) for agents; management Bearer keys can call `GET /api/projects`, `POST /api/projects`, and `POST /api/projects/{project_id}/api-keys` to self-serve projects and project API keys while dashboard billing/destructive account actions remain browser-authenticated.
- Production access note (May 22, 2026): the production host is reachable with `ssh root@178.156.219.85`; production deploy/config files live under `/root/retriever.sh`, including `.env`, `.deploy.env`, `docker-compose.yml`, and the synced `vespa/` package. On this Arch workstation, use `ssh -F /dev/null root@178.156.219.85` if the local system SSH config permission issue blocks the default command.
- Backend hang mitigation (Jun 21, 2026): production was observed with a single Uvicorn worker accepting TCP connections but not responding to `/livez`, while Docker healthcheck `curl` children accumulated because the healthcheck command had no curl-level timeout. Docker images now run Uvicorn with `WEB_CONCURRENCY` defaulting to 2, bound healthcheck curl with `--connect-timeout 1 --max-time 3`, install `postgresql-client` for `pg_dump`, and guard scheduled background loops with a file lock so only one worker runs backups/token cleanup.
- Item date filtering (Jun 21, 2026): RAG item ingest accepts optional `date` as the application/content timestamp, stores it separately from `created_at`, indexes it in Vespa as the numeric `item_date` attribute, and query accepts inclusive `date_from`/`date_to` filters against that item date.
- Logfire FastAPI instrumentation freeze (Jun 21, 2026): production froze on `/livez` with `LOGFIRE_ENABLED=true` after OpenTelemetry FastAPI instrumentation raised `AttributeError: '_IncludedRouter' object has no attribute 'path'` for unmatched routes. Whole-app `logfire.instrument_fastapi` is disabled; explicit Logfire spans and SQLAlchemy instrumentation remain.
- Agent skill distribution cutover (Jun 21, 2026): the Retriever agent skill now lives canonically in `AndrewMead10/retriever-sh-skill` with a root `SKILL.md` for `npx skills add AndrewMead10/retriever-sh-skill`. This repo consumes it as the `skills/retriever-sh` Git submodule, docs point users at skills.sh install, and deploy checkout initializes submodules.
- RAG query DB lock fix (Jun 21, 2026): project API key auth no longer updates `last_used_at` on every request, and query rate-limit consumption now commits before remote embedding/Vespa awaits. The prior flow held `project_api_keys`/`rate_limit_buckets` row locks across external awaits, so concurrent queries blocked inside sync SQLAlchemy calls and wedged all async workers.
- Rate-limit 429 lock fix (Jun 21, 2026): a burst of queries could exhaust the query bucket, raise HTTP 429 while holding the `rate_limit_buckets ... FOR UPDATE` transaction, and block both Uvicorn workers before dependency teardown released the lock. `consume_rate_limit` now rolls back immediately on exceptions, DB dependencies rollback on exception, and RAG ingest/query commit the rate-limit transaction before external embedding/Vespa awaits.
