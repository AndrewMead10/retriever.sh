...
- Vespa int8 binary embeddings (Feb 13, 2026): `rag_document.embedding` now stores packed binary vectors as `tensor<int8>(x[32])` using `input embedding | binarize | pack_bits`, nearest-neighbor distance switched to `hamming`, and backend query vectors now pack 256 float dims into signed int8 bytes before calling `/search`.
