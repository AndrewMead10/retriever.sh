...
- Polar upgrade flow fix (Mar 18, 2026): existing subscribers no longer start a brand-new Polar checkout when switching plans, because Polar rejects that with "you already have a subscription." Backend billing now calls `subscriptions.update(product_id=...)` for existing subscriptions, updates local `plan_id` + rate-limit buckets immediately, and webhook subscription sync can resolve users by `polar_subscription_id`/`polar_customer_id` when metadata is absent.
